...
import frc.robot.util.AngleCalculations
...
class ArmSystem : SubsystemBase() {
    private val tiltSolenoid = Solenoid(ArmConstants.pcm, PneumaticsModuleType.CTREPCM, ArmConstants.tiltSolenoid)
    private val armAngleMotor = CANSparkMax(ArmConstants.armAngleMotor, CANSparkMaxLowLevel.MotorType.kBrushless)
    private val extensionMotor = CANSparkMax(ArmConstants.extensionMotor, CANSparkMaxLowLevel.MotorType.kBrushless)

    private val armAngleEncoder = DutyCycleEncoder(ArmConstants.armAngleEncoder)
    private val extensionEncoder = Encoder(4, 5)

    val extensionInBeamBreak = DigitalInput(ArmConstants.extensionInBeamBreak)
    val extensionMidBeamBreak = DigitalInput(ArmConstants.extensionMidBeamBreak)
    val extensionOutBeamBreak = DigitalInput(ArmConstants.extensionOutBeamBreak)
...
//        get() = extensionRetractedSwitch.get()
    ...
    val armAngleDegrees
        get() = AngleCalculations.wrapAroundAngles(armAngleEncoder.absolutePosition * 360 - ArmConstants.ARM_ANGLE_OFFSET)
...
//            armAnglePID.reset(armAngleDegrees)
    ...
    fun run() {
        ...
        if (extensionInBeamBreak.get() && desiredExtensionPosition == 0.0) {
            setExtensionPosition(ArmConstants.ARM_EXTENSION_IN)
...
//        return;
        ...
        desiredExtensionPosition = MathUtil.clamp(desiredExtensionPosition, 0.0, ArmConstants.ARM_EXTENSION_LIMIT)
...
//        extensionMotor.set(ControllerIO.extensionManualControl)
    ...
    fun resetPosition() {
        desiredTilt = false
        desiredArmAngle = ArmConstants.ARM_IN_ANGLE
        desiredExtensionPosition = ArmConstants.ARM_EXTENSION_IN
    }
